...
async function main() {
  ...
  let signalingState: RelayAgentStatus["signalingState"] = "connecting";

  const getStatus = (): RelayAgentStatus => ({
  ...
  console.info(
    `Relay agent booted for ${config.countryCode} on peer ${config.peerId} with advertised proxy ${config.proxyAdvertisedHost}:${config.proxyPort}`,
  );
}
