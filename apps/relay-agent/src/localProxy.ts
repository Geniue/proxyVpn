import { createServer, request as httpRequest, type IncomingMessage, type RequestOptions, type Server as HttpServer, type ServerResponse } from "node:http";
...
export function startLocalProxyServer(port: number, host = "127.0.0.1"): HttpServer {
  ...
  server.listen(port, host, () => {
    console.info(`Relay agent local proxy listening on http://${host}:${port}`);
  });
