...
export class RelayOrchestrator {
    ...
    getSupportedCountries() {
        return Object.keys(COUNTRY_REGION_MAP);
    }
    async ensureRelay(request) {
    ...
    async ensureRelayInternal(countryCode, requestedWaitMs) {
    ...
    findActiveRelay(countryCode) {
        return (this.options.getPeers().find((peer) => peer.countryCode === countryCode && peer.transport?.mode === "peer-agent") ?? null);
    }
    async findManagedInstance(ec2Client, countryCode) {
        const filters = [
    ...
    buildRunInstancesInput(countryCode, launchConfig) {
        const tags = [
        ...
        if (!launchConfig.imageId || !launchConfig.subnetId || launchConfig.securityGroupIds.length === 0) {
            throw new Error(`Missing AWS launch configuration for ${countryCode}. Configure a launch template or set RELAY_AWS_AMI_ID_${countryCode}, RELAY_AWS_SUBNET_ID_${countryCode}, and RELAY_AWS_SECURITY_GROUP_IDS_${countryCode}.`);
        }
    ...
    buildUserData(countryCode, launchConfig) {
        return [
            "#!/bin/bash",
            "set -euxo pipefail",
    ...
    loadLaunchConfig(countryCode) {
        ...
        return {
            region,
            instanceType: (process.env[`RELAY_AWS_INSTANCE_TYPE_${countryCode}`] ?? process.env.RELAY_AWS_DEFAULT_INSTANCE_TYPE ?? "t3.micro"),
            signalingUrl,
    ...
    async waitForRelayRegistration(countryCode, requestedWaitMs) {
...
function normalizeCountryCode(countryCode) {
    ...
    return normalized in COUNTRY_REGION_MAP ? normalized : null;
}
